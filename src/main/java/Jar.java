...
public class Jar{
  ...
  //    public boolean getmHit(){
   ...
   public boolean isValidGuess(int guess){
       ...
       if (guess <= 0 ){
       throw new IllegalArgumentException("Guess cannot be 0 or Negative");
       }
   ...
   public boolean isHit(){
    ...
    if(mGuess !=mActualNumber){
      mTries--;
      if (mGuess > mActualNumber){
        throw new IllegalArgumentException("Wrong Guess, Too High !");
      } else{
          throw new IllegalArgumentException("Wrong Guess, Too Low !"); 
      }
    } else{
